                ...
                case CAMERA_TYPE_IIDC
                    ...
                    if movie.data_depth == 12
                        ...
                        for i=1:N_frames_to_load
                            data(i).magic           = fread(fid,1,'*uint32');
                            data(i).version         = fread(fid,1,'*uint32');
                            data(i).type            = fread(fid,1,'*uint32');
                            data(i).pixelmode       = fread(fid,1,'*uint32');
                            data(i).length_header   = fread(fid,1,'*uint32');
                            data(i).length_data     = fread(fid,1,'*uint32');
                            data(i).i_guid          = fread(fid,1,'*uint64');
                            data(i).i_vendor_id     = fread(fid,1,'*uint32');
                            data(i).i_model_id      = fread(fid,1,'*uint32');
                            data(i).i_video_mode    = fread(fid,1,'*uint32');
                            data(i).i_color_coding  = fread(fid,1,'*uint32');
                            data(i).i_timestamp_us  = fread(fid,1,'*uint64');
                            data(i).i_size_x_max    = fread(fid,1,'*uint32');
                            data(i).i_size_y_max    = fread(fid,1,'*uint32');
                            data(i).i_size_x        = fread(fid,1,'*uint32');
                            data(i).i_size_y        = fread(fid,1,'*uint32');
                            data(i).i_pos_x         = fread(fid,1,'*uint32');
                            data(i).i_pos_y         = fread(fid,1,'*uint32');
                            data(i).i_pixnum        = fread(fid,1,'*uint32');
                            data(i).i_stride        = fread(fid,1,'*uint32');
                            data(i).i_data_depth    = fread(fid,1,'*uint32');
